            ...
            <span v-if="drop">
              ...
              <Form-item label="商品分类" prop="type">
                <Select v-model="searchForm.type">
                  <Option
                    v-for="(item, i) in this.$store.state.dict.commodityType" clearable style="width: 200px"
                    :key="i"
                    :value="item.value"
                  >{{item.title}}</Option>
                </Select>
...
import { getCommodityList, deleteCommodity } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
  ...
  methods: {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteCommodity({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
            deleteCommodity({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
